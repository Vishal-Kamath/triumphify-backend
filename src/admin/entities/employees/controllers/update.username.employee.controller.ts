...
import { EmployeeUsername } from "../validators.employees";
...
const handleUpdateEmployeeUsername = async (
  req: Request<{}, {}, EmployeeUsername & TokenPayload>,
  res: Response
) => {
  try {
    ...
    await db.update(employee).set({ username }).where(eq(employee.id, id));
    ...
    return res.status(200).send({
      title: "Success",
      description: "Employee username updated successfully",
      type: "success",
    });
  } catch (err) {
...
export default handleUpdateEmployeeUsername;
